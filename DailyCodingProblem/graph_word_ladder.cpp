...
{
	Solution sol;
	vector<string> word_list{"dot", "dog", "dat","cat"};
	string begin = "dog", end = "cat"; 
	
	vector<vector<string>> result = sol.findLadders(begin, end, word_list);
	
	for (vector<string> ss : result)
	{
		for (string s : ss)
			cout << s << " ";
		cout << endl;
