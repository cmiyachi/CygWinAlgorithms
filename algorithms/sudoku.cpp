...
bool sudokuSolve( const vector<vector<char>>& const_board ) 
{
    vector<vector<char>> board = const_board; 
...
{ 
   ...
   bool is_used = !usedInRow(board, row, num) && 
           !usedInCol(board, col, num) && 
           !usedInBox(board, row - row%3 , col - col%3, num);
	bool is_open =(board{row}{col}=='.'); 
    return is_open && is_used; 
