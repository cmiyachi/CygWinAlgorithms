...
private:
	...
	else temp = length; /*cout << "A longest common substring for \""
                  << s1 << "\" and \"" << s2 << "\" is " << "\""
...
int longestCommonSubstring(std::string s, std::string t) {
	...
	// (new LongestCommonSubstring('a','z'))->run(s,t);
	LongestCommonSubstring lcs = LongestCommonSubstring('a','z'); 
	lcs.run(s,t);
	return lcs.length; 
	// return longest; 
